...
- 而**当时读出来的「分支规则没有拦住任何东西」是错的**（同日更晚用干净测量推翻，详见另一份记忆的「再更正」）：实测 `dev` 与 `fix/*` 的推送都不创建部署。当日那 111 条记录里绝大多数**早于配置落地**（13:20 才进 main），用它们推断规则行为本身就不成立。仍然解释不了的是 `fix/159-*` 在 15:54 留下的两条 `CANCELED` 记录 —— 没有复现，无结论。
- 另外，当时这张表的分布是：`dev`、`feat/118-avatar`、`feat/107-home-dashboard`、`refactor/122-login-ui`、`fix/115-*`、`fix/159-*` 各自都有部署记录（两个项目各一份），尽管配置里写着这些分支为 `false`。`fix/159-*` 那次是在额度恢复、且提交树里**确实带着**该配置的情况下仍然部署的。
