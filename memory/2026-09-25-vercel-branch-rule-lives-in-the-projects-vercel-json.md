...
## 再更正（同日最晚，额度可用时的干净测量）：规则**是生效的**

前面那个「更正」说「至少没有按文档所说的方式拦住部署」，那是错的。额度恢复、配置在提交树里、且**没有别的变量**的情况下做了两次干净测量：

| 动作 | 两个项目 |
| --- | --- |
| 推 `dev`（`4608d6c`，改了 `apps/web`） | 无部署记录、无 GitHub 状态 |
| 推 `fix/173-arrow-and-guest-card`（三个提交，改了 `apps/web`） | 无部署记录、无 GitHub 状态 |
| 为它建 PR #173 | 仍然无部署记录、无状态 |

也就是说 `dev: false` 与 `fix/*: false` 都**按文档生效**了。原始结论（配置要写在项目自己的 `vercel.json`、也就是 Root Directory 下那份）成立。
