...
| **卡片里放什么** | **标签、值、动作、入口**，四样，且**游客与登录后完全同构**（账号卡：`账号` / 邮箱或`未登录` / 开始训练或登录 / 我的账号 / 收藏夹 · 错题本）。不放解释句 —— 按钮已经说了它做什么，进度条也说了剩多少；写完先问一句：这句删掉，读者还能看出该干什么吗，能就删。但要小心删过头：**一边有标题、另一边没有，比多两行字更难看**（这个坑踩过一次：删标签时把两态删成了不对称） |
