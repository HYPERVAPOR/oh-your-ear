...
| **空状态** | 一句话说下一步做什么，不道歉 |
| **头像（avatar）** | **一律方角**，40px（账号卡）/ 64px（`/me` 头部）。三来源按优先级：用户上传的 > 登录方式给的（Google `picture`）> **生成的像素头像**（从账号 id 派生的 5×5 镜像图案，墨色实心格 + `canvas-soft` 底，**不用模块色** —— 那五个色是练习模块的身份）。同一账号永远同一张，不落库、不占接口。上传的图在浏览器里裁方缩放（256×256 JPEG），服务端只验不解码重编码。游客的位置留一个空的 1px 发丝线方框：登录不该让卡片跳。 |
