...
| **卡片里放什么** | 账号卡**登录后四样齐全**：标签（`账号`）、值（头像 + 邮箱）、动作（开始训练 + 我的账号）、入口（收藏夹 · 错题本）。**游客态不是这张卡** —— 没有账号时，标签、头像框、「未登录」全都不成立，卡片里只居中放一个登录按钮（底部的两个入口留着，它们登录后才有用）。卡片尺寸两态一致（高度由旁边热力图决定），所以整页不动。不放解释句 —— 按钮已经说了它做什么，进度条也说了剩多少；写完先问一句：这句删掉，读者还能看出该干什么吗，能就删。但要小心删过头：**一边有标题、另一边没有，比多两行字更难看**（这个坑踩过一次：删标签时把两态删成了不对称） |
