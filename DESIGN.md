...
| **进度条** | 4px（`h-1`），底 `surface-strong`，填充 ink（`bg-primary`），不弄圆角。数字用等宽字，靠右或跟在标题后。**它只用于「已通关数 / 总关卡数」**；每日进度由热力图的日档承担，不再另画一根 |
...
| **热力图** | 三档：达标 `bg-success`、部分 `bg-success/35`、没练 `bg-surface-strong`，方块 16px、间距 2px —— **日 / 周 / 月 共用这一把尺子**（一格就是一份进度：今天的一题、一周的一天、一个月的一天），**年 是唯一的第二个倍率**：371 天要塞进一张卡，所以它的格子随卡片宽流式变化（`minmax(8px, 1fr)`，桌面上 8px、宽卡长到 10px 封顶，保底 8px 之下宁可横向滚动）。**日档与周档铺满盒子的宽度、格子均分**（两者长度相同，星期名因此舒展），月档保持紧凑日历页；**四个视图的左边缘一律对齐**（年视图原来的星期名竖栏和 32px 缩进已去掉，好让左边缘都在 0）；星期只有横向一种写法（周 / 月），月份刻度只出现在年。四个 tab 是**同一张日历的四种倍率** —— 日 = 当天目标的进度格（一格一题，`0/20 → 20/20`）、周 = 本周七天、月 = 本月日历、年 = 一年热力图。**格子永远是「一天」**（日档是「一题」），颜色永远问的是「那天我做到目标了吗」，没有相对强度这回事。星期缩写一律英文（`Mon`…`Sun`）：那是刻度，不是句子。**四档共用同一个固定高度的身体**（按最高的年档定），切 tab 不许让卡片跳高矮；**日档即今日进度本身**，别处不许再画一遍 |
