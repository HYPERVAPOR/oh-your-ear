...
| **进度条** | 底 `surface-strong`，填充 ink（`bg-primary`），不弄圆角，数字用等宽字。**两处用它**：闯关格子里「已通关数 / 总关卡数」是 4px（`h-1`）；热力图日档的今日进度是 8px（`h-2`）—— 那个条是整个视图的主角，其余地方一律 4px |
