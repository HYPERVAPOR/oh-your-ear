...
| **空状态** | 一句话说下一步做什么，不道歉 |
| **头像（avatar）** | **一律方角**，40px（账号卡）/ 64px（`/me` 头部）。三来源按优先级：用户上传的 > 登录方式给的（Google `picture`）> **默认位图**（`public/avatar-default.png`，160×160，白底黑像素的 **5×5 镜像方块** —— GitHub identicon 那种样子；**不要人形剪影**，那会踩到恐怖谷）。只有纯黑与纯白两色。**不随主题切换、也不按账号生成** —— 头像不该因为明暗主题或 hash 换个意思；不落库、不占接口。上传的图在浏览器里裁方缩放（256×256 JPEG），服务端只验不解码重编码。游客的位置留一个空的 1px 发丝线方框：登录不该让卡片跳。 |
