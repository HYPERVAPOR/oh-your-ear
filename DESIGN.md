...
| **空状态** | 一句话说下一步做什么，不道歉 |
| **头像（avatar）** | **一律方角**，40px（账号卡）/ 64px（`/me` 头部）。三来源按优先级：用户上传的 > 登录方式给的（Google `picture`）> **生成的 identicon**（5×5 左右镜像方块，图案与色相都来自账号 id 的 hash，跟 GitHub 看齐；**不要人形剪影** —— 40px 的黑色半身像会踩恐怖谷，这是实测过的）。**颜色 inline 写死在元素上，不跟主题走**：头像不该因为明暗主题换个意思。这是全站**唯一允许出现的非调色板颜色** —— 它是账号的身份，不是我们的配色；不落库、不占接口。上传的图在浏览器里裁方缩放（256×256 JPEG），服务端只验不解码重编码。游客的位置留一个空的 1px 发丝线方框：登录不该让卡片跳。 |
