...
| **热力图** | 三档：达标 `bg-success`、部分 `bg-success/35`、没练 `bg-surface-strong`，方块一律 10px、间距 2px。四个 tab 是**同一张日历的四种倍率** —— 日 = 当天目标的进度格（一格一题，`0/20 → 20/20`）、周 = 本周七天、月 = 本月日历、年 = 一年热力图。**格子永远是「一天」**（日档是「一题」），颜色永远问的是「那天我做到目标了吗」，没有相对强度这回事。星期缩写一律英文（`Mon`…`Sun`）：那是刻度，不是句子。**四档共用同一个固定高度的身体**（按最高的年档定），切 tab 不许让卡片跳高矮；**日档即今日进度本身**，别处不许再画一遍 |
