...
| **棋盘（rack）** | 一格一件事：`grid-cols-2 gap-2`（宽屏**五列**），每格上方一条 3px 模块色块。首页只有一个棋盘 —— 五个模块，Learn / Random 两个 tab 切换的是同一批模块的两种去处（带进度条的关链 / 不带进度条的随机练）。**收藏夹与错题本不在棋盘里**：它们属于账号，入口在账号卡下缘那条发丝线之下。**落地页第二屏的账号卡片也用这套**（五格、宽屏五列、**无 3px 色块** —— 那五个色是练习模块的身份，这五项不是模块） |
