...
- **issue**: #107
- **status**: 🟡 doing
- **description**: 热力图加日 / 周 / 月 / 年四个 tab，都是同一张日历的四种倍率，数据只有 `GET /me/daily` 一个来源，不新增接口：日档是当天目标的进度格（一格一题，`0/20 → 20/20`），周档是本周七天，月档是本月日历（30/31 个格子，随年份），年档是一年的热力图。**格子永远是一天，永远按那天自己的目标分档**，没有相对强度。纵轴星期缩写用英文。四个函数（`goalSlots` / `week` / `month` / `year`）在 `lib/heatmap.ts`，带表驱动单测。
- **depends on**: 15.2
