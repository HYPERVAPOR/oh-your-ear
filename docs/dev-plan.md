...
- **issue**: #193
- **status**: 🟡 doing
- **description**: `focus_exercises = []` 的语义是「不缩小范围」，但表单把空列表画成**五个都没亮**，而保存只校验题数 —— 用户能存下「一个都不选」，界面显示全灭、实际行为是五个都用。现在：语义不动（`[]` = 全部，API 与已有数据都不动），**表单进来就五个全亮**，全灭时禁用保存并说明理由（`plan.pickOne` / `plan.goalRange`）。`/me` 上「去练今天的一组」改指 `/daily`。重点模块那五个格子重新设计：等宽等高、各带自己的**模块色圆点**（与关卡页选项卡、统计图同一套 `ModuleSwatch`），**选中 = 墨色文字 + 该模块色 2px 下划线**（关卡页那只手柄），未选 = muted 文字、无下划线 —— 先做过「选中铺该色底色 + 描边」，读者判断太张扬，退回手柄式。`auto-rows-fr` + 宽屏五列 / `md` 以下两列，实测中英 1200px 五格各 209.4px、390px 各 134.5px（互差 <0.1px），下划线贴底、宽 = 格宽，深色换深色那套模块色，无横向滚动。
